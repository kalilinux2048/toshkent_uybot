...
async def add_listing(region, district, category, title, price, rooms, description, phone, image_url=None, media_group=None):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
async def get_all_listings(district, category):
    ...
    WHERE district LIKE ? AND category LIKE ? AND status='active'
